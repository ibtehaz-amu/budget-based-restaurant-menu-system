   ...
   export default function Register() {
     ...
     const handleRegister = async (e) => {
       ...
       try {
         ...
         const response = await fetch('https://budget-based-restaurant-menu-system.onrender.com/api/auth/register', {
           method: 'POST',
           headers: {
           ...
           if (data.token && !data.token.includes('fake') && !data.token.includes('demo')) {
             ...
             if (formData.role === 'OWNER' && formData.restaurantName && formData.restaurantAddress) {
               try {
                 ...
                 const userResponse = await fetch('https://budget-based-restaurant-menu-system.onrender.com/api/auth/test-token', {
                   method: 'GET',
                   headers: {
                 ...
                 if (userResponse.ok) {
                   ...
                   const restaurantResponse = await fetch('https://budget-based-restaurant-menu-system.onrender.com/api/restaurants', {
                     method: 'POST',
                     headers: {
                       'Content-Type': 'application/json',
                       'Authorization': `Bearer ${data.token}`
     ...
     return (
       <div className="min-h-screen flex items-center justify-center bg-gray-100 py-12 px-4">
         <div className="bg-white p-8 rounded-lg shadow-md w-full max-w-md">
           {/* ✅ FIXED: Changed text color to dark */}
